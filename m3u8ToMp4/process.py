...
def get_speed_displayed(start_time, download_end_time, end_time, file_path, name):
    ...
    print("【INFO】 视频下载成功！ 一共耗时：%ss  平均下载速度：%sKB/s" %
...
class Crawler:
        ...
        if rsp.text.find(".m3u8") != -1:
            ...
            if len(m3u8_ls) == 1:
                ...
                if self.auto_bitrate:
                    choice = len(
                        m3u8_ls) if self.auto_bitrate_level == "HIGHEST" else 1
    ...
    def _ts_is_encrypted_judge(self, rsp, prefix):
        ...
        if rsp.text.find("EXT-X-KEY") != -1:
            ...
            print("【INFO】 ts文件已被加密  加密算法: " + self.encrypt_method +
                  "  加密文件名: " + self.encrypt_file)
    ...
    def _collect_ts_files(self, rsp, protocol, host, prefix):
        ...
        for index, item in enumerate(ts_ls):
            ts_ls[index] = assemble_new_request(
                ts_ls[index], protocol, host, prefix)
    ...
    def download_ts_files(self, ts_ls):
        ...
        file_path = os.path.join(os.path.join(
            self.download_base_path, self.name))
        ...
        if self.use_ip_proxy_pool:
            ...
            proxies_ip = proxies_generate.get_proxies_ip(
                type_, self.total_clips)
            print("【INFO】 IP代理准备完成,已待命IP代理数为: " + str(len(proxies_ip)))

        utils.progress_bar(0, len(ts_ls), prefix="ts文件集",
                           suffix="下载中...", completed_suffix="ts文件集下载成功!")

        download_queue = []

        # 这个1不可省略，因为下面的判断为 <self.completed_queue_length，不+1会导致有一个切片一直无法请求
        index = -self.download_queue_length + 1
        ...
        while self.current_clips < self.total_clips:
            while index < self.completed_queue_length:
                ...
                ts_name = item[item.rfind(
                    '/') + 1: item.rfind(self.ts_suffix)] + "ts"
    ...
    def download_encrypt_file_handler(self, encrypt_url):
        ...
        try:
            self.encrypt_key = request_get(
                encrypt_url, self.tries, timeout=5).text
            print("【INFO】 获取到加密密钥")
        except RequestError:
    ...
    def download_ts_handler(self, ts_url, file_path, file_name, proxy_ip=None):
        ...
        try:
            if self.use_ip_proxy_pool:
                rsp = request_get(ts_url, self.tries,
                                  timeout=5, proxy_ip=proxy_ip)
            else:
            ...
            with open(os.path.join(file_path, file_name), 'wb') as file:
                if self.encrypt_method is None:
                    ...
                    cryptor = crypt_obj.new(
                        self.encrypt_key.encode("utf-8"), crypt_obj.MODE_CBC)
                    file.write(cryptor.decrypt(rsp.content))

            self.current_clips += 1
            self.completed_queue_length += 1

            utils.progress_bar(self.current_clips, self.total_clips, decimals=2,
                               prefix="ts文件集", suffix="下载中...", completed_suffix="ts文件集下载成功")
    ...
    def merge_and_delete_ts_set(self, file_path, ts_name_ls):
        ...
        get_speed_displayed(self.start_time, self.download_end_time,
                            self.end_time, file_path, self.name)

    def _merge_set(self, file_path, ts_name_ls):
        """ 存入ts文件名称顺序，并用ffmpeg进行视频文件合成 """
        with open(os.path.join(file_path, self.name + ".mp4"), 'wb') as merged:
            for item in ts_name_ls:
                with open(os.path.join(file_path, item), 'rb') as mergefile:
    ...
    def move_video_higher(self, file_path):
        shutil.move(os.path.join(file_path, self.name + ".mp4"),
                    os.path.join(self.download_base_path, self.name + ".mp4"))
